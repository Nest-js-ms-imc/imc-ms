...
describe('EnvsService', () => {
  ...
  describe('loadSecrets', () => {
    it('should call loadSecrets in onModuleInit', async () => {
      const mockSecret =
        "'{PORT:3000,DB_USERNAME:TEST_USERNAME,DB_PASSWORD:TEST_PWD,DB_NAME:TEST_DB,DB_HOST:localhost,DB_PORT:54312}'";
    ...
    it('Should throw error', async () => {
      ...
      await expect(envsService.loadSecrets()).rejects.toThrow(
        'No se pudieron obtener los secretos de AWS',
      );
    });
  ...
  describe('get', () => {
    it('should return the correct value of an environment variable', async () => {
      const mockSecret =
        "'{PORT:3000,DB_USERNAME: TEST_USERNAME,DB_PASSWORD:TEST_PWD,DB_NAME:TEST_DB,DB_HOST:TEST_DB_HOST,DB_PORT:5431}'";
